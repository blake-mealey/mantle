...
struct RobloxApiError {
    ...
    title: Option<String>, // There are some other possible properties but we currently have no use for them so they are
                           // not included
}
...
fn main() {
    ...
    let result = match matches.subcommand() {
        ...
        ("deploy", Some(deploy_matches)) => command_deploy(
