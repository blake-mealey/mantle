...
```yaml filename="project/mantle.yml"
target:
  experience:
    configuration:
...
assets (only for versions above `0.11.0`). You can also use the schema to add autocomplete to your editor. For
...
```json
"yaml.schemas": {
   "https://mantledeploy.vercel.app/schemas/{{ schemaVersion }}/schema.json": "mantle.yml"
}
```
